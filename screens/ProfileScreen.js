...
const ProfileScreen = () => {
  ...
  useEffect(() => {
    const loadProfileData = async () => {
      try {
        ...
        Alert.alert(t.error || "Error", t.loadProfileError || "Failed to load profile data.");
      }
    };
  ...
  const handleSave = async () => {
    ...
    try {
      const db = await getDb();
      const user = await db.getFirstAsync(`SELECT id FROM Users WHERE email = ?`, [data.email]);
      if (user) {
        await db.runAsync(`UPDATE Users SET fullName = ? WHERE email = ?`, [data.fullName, data.email]);
      } else {
        await db.runAsync(
      ...
      Alert.alert(t.error || "Error", t.saveProfileError || "Failed to save profile data.");
    }
  };

  const handlePrivacyPolicy = () => {
    Linking.openURL("https://www.who.int/about/policies/privacy").catch((err) =>
      Alert.alert(t.error || "Error", t.errorPrivacyPolicy || "Failed to open privacy policy.")
    );
  };
        ...
        { text: t.cancel || "Cancel", style: "cancel" },
        {
          ...
          onPress: async () => {
            try {
              ...
              await db.runAsync("DELETE FROM UserProfile WHERE email = ?", [email]);
              console.log("Account data (AsyncStorage and UserProfile) deleted successfully");
              navigation.reset({
                index: 0,
                routes: [{ name: "Welcome" }],
              });
              ...
              Alert.alert(t.error || "Error", t.deleteAccountError || "Failed to delete account data.");
            }
          },
        },
      ]
        ...
        { text: t.cancel || "Cancel", style: "cancel" },
        {
          ...
          onPress: async () => {
            try {
              ...
              Alert.alert(t.error || "Error", t.logoutError || "Failed to log out.");
            }
          },
        },
      ]
    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView style={{ flex: 1 }} showsVerticalScrollIndicator={false}>
        <View style={styles.headerContainer}>
          <View style={styles.headerContent}>
            ...
            <Text style={styles.appTagline}>
              {t.profileTagline || "Manage your personal information and settings"}
            </Text>
          </View>
        </View>
        ...
        <View style={styles.scrollContent}>
          ...
          <View style={styles.section}>
            ...
            <View style={styles.settingsCard}>
              <View style={styles.pickerSection}>
                <Text style={styles.inputLabel}>{t.language || "Language"}</Text>
                <TouchableOpacity
                  style={styles.pickerButton}
                  onPress={() => setShowLanguageModal(true)}
                >
                  <Text style={styles.pickerButtonText}>{getCurrentLanguageName()}</Text>
          ...
          <View style={styles.section}>
            <Text style={styles.sectionTitle}>{t.securityPrivacy || "Security & Privacy"}</Text>
            <View style={styles.securityCard}>
              <View style={styles.securityItem}>
                ...
                <View style={styles.securityContent}>
                  <Text style={styles.securityTitle}>{t.dataEncryption || "Data Encryption"}</Text>
                  <Text style={styles.securityDescription}>
                    {t.encryptionDesc || "Your data is encrypted and secure."}
                  </Text>
                </View>
                <View style={styles.securityStatus}>
                  <Text style={styles.activeStatus}>{t.active || "Active"}</Text>
                </View>
              </View>
            </View>
          ...
          <View style={styles.actionSection}>
            <TouchableOpacity style={styles.primaryButton} onPress={handleSave}>
              <Text style={styles.primaryButtonText}>{t.saveChanges || "Save Changes"}</Text>
            </TouchableOpacity>
            <TouchableOpacity style={styles.dangerButton} onPress={handleDeleteAccount}>
              <Text style={styles.dangerButtonText}>{t.deleteAccount || "Delete Account"}</Text>
            </TouchableOpacity>
            <TouchableOpacity style={styles.secondaryButton} onPress={handleLogout}>
              <Text style={styles.secondaryButtonText}>{t.logOut || "Log Out"}</Text>
            </TouchableOpacity>
          </View>
        ...
        >
          <View style={styles.modalContent}>
            ...
            {availableLanguages.map((lang) => (
              <TouchableOpacity
                key={lang.code}
                style={[styles.modalOption, language === lang.code && styles.modalOptionSelected]}
