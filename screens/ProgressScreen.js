...
const ProgressScreen = () => {
  ...
  const fetchProgressData = useCallback(async () => {
    ...
    try {
      ...
      const storedHistoricalData = await AsyncStorage.getItem("historicalHealthData");

      // Use navigation params first, then stored data as fallback
      const currentLifestyleData = navLifestyleData || (storedProfileData ? JSON.parse(storedProfileData) : null);
      const currentPredictionData = navPredictionData || (storedPredictionData ? JSON.parse(storedPredictionData) : null);

      console.log("Progress Screen - Current lifestyle data:", currentLifestyleData);
      console.log("Progress Screen - Current prediction data:", currentPredictionData);
      ...
      if (currentLifestyleData) {
        currentDayData = {
          ...
          exercise_frequency: Number(currentLifestyleData.Exercise_Frequency) || 0,
          ...
          screen_time_hours: Number(currentLifestyleData.Screen_Time_Hours) || 0,
          salt_intake: currentLifestyleData.Salt_Intake || "Moderate",
        };
      }

      // Get historical data
      const parsedHistoricalData = storedHistoricalData ? JSON.parse(storedHistoricalData) : [];
      ...
      historicalData.forEach(item => {
        const existingIndex = allData.findIndex(existing => existing.date === item.date);
        if (existingIndex >= 0) {
      ...
      if (currentDayData) {
        const existingIndex = allData.findIndex(existing => existing.date === currentDayData.date);
        if (existingIndex >= 0) {
      ...
      setProgressData(allData.sort((a, b) => new Date(a.date) - new Date(b.date)));
    } catch (error) {
      ...
      <View style={styles.quickStatsCard}>
        ...
        {isLoading ? (
          ...
          <View style={styles.compactStatsGrid}>
            ...
            <View style={styles.compactStatItem}>
              <Text style={styles.compactStatValue}>
                {Math.round(summaryStats.riskLevel)}{t.percentSymbol || "%"}
              </Text>
